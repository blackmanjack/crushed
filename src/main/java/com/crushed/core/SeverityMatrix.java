    ...
    public static Severity severityFor(IssueType issueType) {
        return switch (issueType) {
            case MASS_ASSIGNMENT, RCE, SQL_INJECTION, SSTI, SSRF, IDOR_BOLA, XXE, AI_SESSION_MISCONFIGURATION, PATH_TRAVERSAL,
                    INSECURE_DESERIALIZATION, FORBIDDEN_BYPASS ->
                    Severity.HIGH;
            case FIREBASE_MISCONFIGURATION, DOM_XSS, REFLECTED_XSS, JWT_WEAKNESS, CSRF, CRLF_INJECTION, OAUTH_MISCONFIGURATION,
                    REQUEST_SMUGGLING_CANDIDATE, PROTOTYPE_POLLUTION ->
                    Severity.MEDIUM;
            case CORS_MISCONFIGURATION, OPEN_REDIRECT, UNICODE_NORMALIZATION, RACE_CONDITION_CANDIDATE,
                    COOKIE_MISSING_SECURE_FLAG, COOKIE_MISSING_HTTPONLY_FLAG, COOKIE_MISSING_SAMESITE,
                    MISSING_HSTS, MISSING_CLICKJACKING_PROTECTION ->
                    Severity.LOW;
            case GRAPHQL_INTROSPECTION, SENSITIVE_INFO_DISCLOSURE, REFLECTED_INPUT, TECH_STACK_DISCLOSURE,
                    MISSING_X_CONTENT_TYPE_OPTIONS, MISSING_CSP ->
                    Severity.INFO;
        };
    }
}
